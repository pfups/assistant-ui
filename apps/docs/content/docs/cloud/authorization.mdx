...
The assistant-ui API can be directly accessed by your frontend. This eliminates the need for a backend server from your side, except for authorization of your users.
...
There are three supported approaches to obtain a workspace auth token:
...
- simplest to setup
- does not require a user to be authenticated
- suitable for public access areas of your application

You can always switch between the three approaches without any downtime or necessary database migrations.
Choose direct integration with your auth provider if you can. Otherwise, use a backend server or anonymous authentication.
...
The anonymous authentication strategy allows you to access the assistant-ui API without requiring a user to be authenticated. This is useful for public access areas of your application.

To use the anonymous authentication strategy, you need to pass in a configuration object with `anonymous: true` and `baseUrl: <your base url>` to the `AssistantCloud` constructor.
...
The `AssistantCloud` instance will automatically handle token management for anonymous users.